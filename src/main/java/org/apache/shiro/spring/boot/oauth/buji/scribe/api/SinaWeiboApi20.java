...
public class SinaWeiboApi20 extends DefaultApi20 {
	...
	public String getAuthorizationUrl(OAuthConfig config) {
		...
		if (config.hasScope()) {
			return String.format(OAuthConstants.SINA_SCOPED_AUTHORIZE_URL, config.getApiKey(), OAuthEncoder.encode(config.getCallback()), OAuthEncoder.encode(config.getScope()));
		} else {
			return String.format(OAuthConstants.SINA_AUTHORIZE_URL, config.getApiKey(), OAuthEncoder.encode(config.getCallback()));
		}
	}
